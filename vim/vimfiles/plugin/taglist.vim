...
function! s:Tlist_Window_Exit_Only_Window()
    " Smart quit tag_list_window
    function! s:Tlist_Smart_quit()
        bdelete
        try
            quit
        catch /^Vim\%((\a\+)\)\=:E173/	" catch E173: 1 more file to edit 
            " ask whether quit
            :echohl Question
            let s:quit_return_string = input("\E173: 1 more file to edit\tQuit ALL?[y/n]")
            if s:quit_return_string =~ "^[yY]$" || s:quit_return_string =~ "[yY][Ee][Ss]" || s:quit_return_string =~ "[oO][kK]"
                quit!
            else
                :echohl ErrorMsg
                echo "\E173: 1 more file to edit"
                :echohl NONE
                " recover the UI
